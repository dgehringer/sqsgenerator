...
namespace sqsgen::core::optimization {
  ...
  template <class T>
  std::tuple<std::vector<usize_t>, std::vector<bounds_t<usize_t>>, std::vector<std::size_t>>
  compute_shuffling_bounds(structure<T> const& structure,
    ...
    auto sublattice_index = [&]<typename T0>(T0&& site) -> usize_t {
      usize_t search;
      if constexpr (std::is_same_v<std::decay_t<T0>, usize_t>)
        search = site;
      else
        search = site.index;
  ...
  template <> double compute_objective(cube_t<double>& sro, cube_t<usize_t> const& bonds,
                                       cube_t<double> const& prefactors,
                                       cube_t<double> const& pair_weights,
                                       cube_t<double> const& target, std::size_t num_shells,
                                       std::size_t num_species);

  template <> float compute_objective(cube_t<float>& sro, cube_t<usize_t> const& bonds,

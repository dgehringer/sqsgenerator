...
namespace sqsgenerator {
    ...
    parameter_storage_t expand_matrix(const parameter_storage_t &matrix, const IterationSettings &settings, const std::vector<int> &reindexer) {
        size_t nspecies {settings.num_species()},
                nshells{settings.num_shells()},
                npars_per_shell_asym {(nspecies * (nspecies -1))/2 + nspecies}, // upper half of a symmetric matrix plus the main diagonal
        ...
        for (size_t shell = 0; shell < nshells; shell++) {
            ...
            for (size_t si = 0; si < nspecies; si++) {
                for (size_t sj = si; sj < nspecies; sj++) {
                    int flat_index {reindexer[si*nspecies+sj]};
                    vec[offset_full+si*nspecies+sj] = vec[offset_compact+flat_index];
                    if (si != sj) vec[offset_full+sj*nspecies+si] = vec[offset_compact+flat_index];
                    assert(flat_index >= 0 && flat_index < static_cast<int>(npars_per_shell_asym));
                }
            }
        }
    ...
    static std::vector<SQSResult> do_iterations_vector(const IterationSettings &settings) {
        ...
        for (size_t i = 0;  i < nshells; i++) std::cout << "do_iterations_vector: \t" << shells[i]  << " : " << shell_weights[i] << std::endl;
        ...
        #pragma omp parallel default(none) shared(std::cout, boost::extents, settings, results, iteration_ranks, hist, nperms, pair_list, prefactors, parameter_weights, target_objectives, reindexer, best_objective, thread_timings) firstprivate(nspecies, nshells, reduced_size)
        {
            ...
            switch (settings.mode()) {
                case iteration_mode::random:
                {
                    #pragma omp critical
                    {
                        ...
                        std::cout << "[THREAD: " << thread_id <<"] random_seed_local = " << random_seed_local << ", time = " << std::time(nullptr) << std::endl;
                    }
            ...
            thread_timings[thread_id] = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
            #pragma omp critical
            std::cout << "[THREAD: " << thread_id << "] finished = " << end_it - start_it << " loops (" << static_cast<double>(thread_timings[thread_id])/(end_it -start_it).convert_to<long>() << " mcirosec/cycle)"  << std::endl;
        } // pragma omp parallel
    ...
    static std::vector<SQSResult> do_iterations(const IterationSettings &settings) {
        ...
        {
            int thread_id {omp_get_thread_num()}, nthreads {omp_get_num_threads()};
            rank_t start_it, end_it, total_iterations;
            uint64_t random_seed_local;
            get_next_configuration_t get_next_configuration;

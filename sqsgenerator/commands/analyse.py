...
@click.command('analyse', help=help.analyse)
@click.option('--output-format', '-of', type=click.Choice(['yaml', 'json', 'pickle', 'native']), default='native', help=help.output_format)
    ...
    if output_format == 'native':
        if pretty:
            ...
            for rank, result in map_values(AttrDict, document.configurations).items():
                ...
                renderables.insert(0, Text.assemble(('Configuration: ', 'bold'), (f'{result.configuration}', 'bold cyan')))

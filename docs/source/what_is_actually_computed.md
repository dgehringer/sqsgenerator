...
```{math} \alpha_{AB} = 1 - \dfrac{N_{AB}}{NMx_Ax_B}.
---
label: wc-sro
---
```
...
```{math} \alpha^k_{AB} = 1 - \dfrac{N_{AB}^k}{NM^kx_Ax_B}.
---
label: wc-sro-multiple-shells
---
```
...
```{math} \alpha^i_{\xi\eta} = 1 - \dfrac{N^i_{\xi\eta}}{NM^ix_{\xi}x_{\eta}}.
---
label: wc-sro-multi-component
---
```
...
```{math} \mathcal{O}(\sigma) = \sum_{k}w^i\sum_{\xi,\eta}\left|\alpha'_{\xi\eta}-\alpha^i_{\xi\eta}\right|
---
...
```{math} T^{\{i,j,k\}}kdf
---
label: number-of-triplets
---
```

The number of all triplets are computed from the input structure before the iteration itself starts.

```{math} \beta^{ijk}_{\xi\eta\zeta} = 1 - \dfrac{N^{ijk}_{\xi\eta\zeta}}{\frac{1}{|\{i,j,k\}|}NT^{\left\{i,j,k\right\}}x_\xi x_\eta x_\zeta}
---
label: triplet-parameters
---
```
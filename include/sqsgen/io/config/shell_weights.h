...
namespace sqsgen::io::config {
  ...
  namespace detail {
    ...
    parse_result<std::size_t> parse_shell_index(auto value) {
      ...
      if constexpr (std::is_same_v<std::decay_t<Document>, pybind11::handle>
        ...
        return accessor<Document>::template get_as<KEY_NONE, std::size_t>(value);
#endif
    ...
    parse_result<shell_weights_t<T>> parse_weights(Document const& doc, auto num_shells) {
      ...
      for (auto&& [si, w] : accessor<Document>::items(doc)) {
        auto r = parse_shell_index<key, Document>(si)
                     ...
                     .and_then([&](auto&& i_and_w) -> parse_result<std::tuple<std::size_t, T>> {
    ...
    template <class T>
    std::vector<shell_weights_t<T>> default_weights(std::vector<std::size_t> const& num_shells) {
      return as<std::vector>{}(num_shells | views::transform([](auto nshells) {
                                 return as<std::map>{}(
                                     core::helpers::range<std::size_t>({1, nshells})
                                     | views::transform([&](std::size_t&& i) {
  ...
  weights_t<T> parse_shell_weights(Document const& document, SublatticeMode mode,
    ...
    const auto num_shells = as<std::vector>{}(shell_radii | views::transform([](auto&& r) {

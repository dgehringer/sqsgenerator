...
namespace sqsgen::io {
  ...
  std::optional<parse_result<Options...>> get_either_optional(Document const& doc) {
    ...
    if (accessor<Doc>::contains(doc, key.data) || key == KEY_NONE) {
      result_t result = {parse_error::from_msg<key, CODE_UNKNOWN>(
          fmt::format("unknown error failed to load {}", key.data))};
  ...
  template <string_literal key, class Value, class Document>
  std::optional<parse_result<Value>> get_optional(Document const& json) {
